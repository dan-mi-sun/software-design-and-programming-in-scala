...
public class State implements Comparable<Object> {
	...
	/**
	 * All possible game States that can result from the next player's Move. The
	 * length of the array equals the number of States. It is an array of length
	 * 0 if there are no possible moves (once it has been set; initially, it is
	 * an array of length0)
	 */
	...
	/**
	 * Constructor: a game State consisting of board b, with player player to
	 * play next; lm is the last Move made on this game --null if the user does
	 * not care what the last move made was.
	 */
	...
	/**
	 * Retrieves the possible moves and initializes this State's children. The
	 * result is that this State's children reflect the possible States that can
	 * exist after the next move. Remember, in the children it is the opposite
	 * player's turn. This method initializes only this State's children; it
	...
	/**
	 * Write this State to a file called "output.txt", including its children,
	 * their children, etc.. This method allows the State to be viewed in a file
	 * even when it is too large to print to console. Beep when printing is
	 * done.
	 */
	 ...
	 * with string ind (expected to be a string of blank characters) followed by
	 * a similar representation of all its children, indented an additional ind
	 * characters. d is the depth of this state.
	 */
